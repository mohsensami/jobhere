        ...
        <div class="user-info bg-white padding-30 font-size-13">
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <div class="profile__info border cursor-pointer text-center">
                    ...
                    <span class="profile__name">کاربر : {{ $user->full_name }}</span>
                    
                    @error('profile')
                      <p class="error">{{ $message }}</p>
                    @enderror
                </div>
                <input class="text" type="text" name="name" placeholder="نام کاربری" value="{{ $user->full_name }}">
                @error('name')
                  <p class="error">{{ $message }}</p>
                @enderror

                <input class="text" type="text" name="mobile" placeholder="شماره تلفن" value="{{ $user->phone }}">
                @error('mobile')
                  <p class="error">{{ $message }}</p>
                @enderror
